# ============================================================================
# Script: setup-registry-v4.ps1
# Objectif: Mettre en place un Registre Docker Local
# Version: Simplifiée, sans sudo, avec vérifications
# ============================================================================
...
Write-Host "SETUP REGISTRE DOCKER LOCAL - V4" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "Solution: Ajouter l'utilisateur au groupe docker: 'sudo usermod -aG docker $MASTER_USER'" -ForegroundColor Yellow
    exit 1
}
...
foreach ($i in 0..1) {
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Write-Host "Solution: Ajouter l'utilisateur au groupe docker: 'sudo usermod -aG docker $worker_user'" -ForegroundColor Yellow
        exit 1
    }
}
...
if ($registry_check -notlike "*OK*") {
    ...
    Write-Host "Vérifiez: kubectl get pods -n kube-system | grep registry" -ForegroundColor Yellow
    exit 1
}
Write-Host "[OK] Registry Docker déployé et accessible" -ForegroundColor Green

# ============================================================================
# STEP 3: Configurer la confiance du Registre sur tous les nodes
# ============================================================================
Write-Host "`n[STEP 3] Configuration du Registre (insecure) sur tous les nodes..." -ForegroundColor Yellow
...
ssh $MASTER_USER@$MASTER_IP "mkdir -p /etc/docker && echo '$daemon_config' > /etc/docker/daemon.json && systemctl daemon-reload && systemctl restart docker && sleep 3 && docker --version"
if ($LASTEXITCODE -ne 0) {
    Write-Host "[ERROR] Impossible de configurer Docker sur le Master" -ForegroundColor Red
    exit 1
}
...
foreach ($i in 0..1) {
    ...
    ssh $worker_user@$worker_ip "mkdir -p /etc/docker && echo '$daemon_config' > /etc/docker/daemon.json && systemctl daemon-reload && systemctl restart docker && sleep 3 && docker --version"
    if ($LASTEXITCODE -ne 0) {
        Write-Host "[ERROR] Impossible de configurer Docker sur le Worker $($i+1)" -ForegroundColor Red
        exit 1
    }
}
...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "Vérifiez la connectivité réseau vers $REGISTRY_URL" -ForegroundColor Yellow
    exit 1
}
