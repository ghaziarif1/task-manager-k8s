...
Write-Host "SETUP REGISTRE DOCKER LOCAL - V2" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan

# ============================================================================
# STEP 1: Créer et exécuter le script de déploiement du Registry
# ============================================================================
Write-Host "`n[STEP 1] Création du script Registry sur le Master..." -ForegroundColor Yellow

$registry_script = @"
#!/bin/bash
set -e

echo '[INFO] Vérification du Registry Docker...'
...
ssh $MASTER_USER@$MASTER_IP "cat > /tmp/setup-registry.sh" < ([System.Text.Encoding]::UTF8.GetBytes($registry_script))
ssh $MASTER_USER@$MASTER_IP "chmod +x /tmp/setup-registry.sh && bash /tmp/setup-registry.sh"
...
# ============================================================================
# STEP 2: Créer et exécuter le script de configuration Docker sur tous les nodes
# ============================================================================
Write-Host "`n[STEP 2] Configuration de la confiance du Registre sur tous les nodes..." -ForegroundColor Yellow

$config_script = @"
#!/bin/bash
set -e

echo '[INFO] Configuration du Registre Docker (insecure)...'
...
ssh $MASTER_USER@$MASTER_IP "cat > /tmp/configure-docker.sh" < ([System.Text.Encoding]::UTF8.GetBytes($config_script))
ssh $MASTER_USER@$MASTER_IP "chmod +x /tmp/configure-docker.sh && bash /tmp/configure-docker.sh"
if ($LASTEXITCODE -ne 0) {
...
foreach ($i in 0..1) {
    ...
    ssh $worker_user@$worker_ip "cat > /tmp/configure-docker.sh" < ([System.Text.Encoding]::UTF8.GetBytes($config_script))
    ssh $worker_user@$worker_ip "chmod +x /tmp/configure-docker.sh && bash /tmp/configure-docker.sh"
    if ($LASTEXITCODE -ne 0) {
...
Write-Host "  → Push Backend vers $tag_backend..." -ForegroundColor Cyan
...
Write-Host "  → Push Frontend vers $tag_frontend..." -ForegroundColor Cyan
...
# ============================================================================
# STEP 6: Vérification du contenu du Registre
# ============================================================================
...
Write-Host "  3. Vérifier que les Pods démarrent" -ForegroundColor White

Write-Host "`nCommandes de diagnostic :" -ForegroundColor Cyan
Write-Host "  # Voir les images du registre" -ForegroundColor White
